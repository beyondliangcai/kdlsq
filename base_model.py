...
from quantized_bert import QuantizedBertConfig
...
class TransformerBase(TrainableModel):
        ...
        optimizer_grouped_parameters = [
            {
                "params": [
                    ...
                    if not any(nd in n for nd in no_decay) and not any(nd in n for nd in alpha)
                ],
            ...
            },
            {
                "params": [
                    ...
                    any(nd in n for nd in no_decay) and not any(nd in n for nd in alpha)
                ],
                "weight_decay": 0.0,
            },
            {
                "params": [
                    p for n, p in self.model.named_parameters() if 'wgt_alpha' in n
                ],
                'lr': 1e-3,  # 'lr': 1e-3,
                "weight_decay": 2e-5,  # 1e-3, 1e-2, defalut(2e-5)
            },
            {
                "params": [
                    p for n, p in self.model.named_parameters() if 'act_alpha' in n
                ],
                'lr': 3e-2,  # 'lr': 3e-2,
                "weight_decay": 1e-4,  # 1e-5, 1e-3, 1e-2, # default (1e-4)
            },
        ]
    ...
    def save_model(self, output_dir: str, save_checkpoint: bool = False, args=None):
        ...
        # call two times that base class and derived class
    ...
    def _train(
            ...
            max_steps: int = -1,
            num_train_epochs: int = 3,  # 3
            max_grad_norm: float = 1.0,
            logging_steps: int = 50,  # 50
            save_steps: int = 1000,
        ...
        # we do initilization for activation alpha here.
        self.model.apply(reset_act_alpha_true)
        self.init_act_alpha(dev_data_set, test_data_set)
        ################################################
            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                if (step + 1) % gradient_accumulation_steps == 0:
                    ...
                    if logging_steps > 0 and global_step % logging_steps == 0:
                        ...
                        print("lr = {}".format(self.scheduler.get_lr()[0]))
    ...
    def init_act_alpha(
        ...
        for i, ds in enumerate([dev_data_set, test_data_set]):
            ...
            if isinstance(ds, DataLoader):
                ds = [ds]
            for d in ds:
                logits, label_ids = self._evaluate_v2(d)
                f1 = self.evaluate_predictions(logits, label_ids)
            break

    def _evaluate_v2(self, data_set: DataLoader):
