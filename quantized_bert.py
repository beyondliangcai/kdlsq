...
QUANT_WEIGHTS_NAME = "quant_pytorch_model.pth" # "quant_pytorch_model.bin"
...
def quantized_linear_setup(config, name, *args, **kwargs):
    ...
    try:
        ...
        quant_config.weight_bits = 2  # 2 bits
...
class QuantizedBertSelfAttention(BertSelfAttention):
    def __init__(self, config):
        ...
        self.activation_query = quantized_activation_setup(config, "activation_query")
        self.activation_key = quantized_activation_setup(config, "activation_key")
        self.activation_value = quantized_activation_setup(config, "activation_value")
        self.activation_att_prob = quantized_activation_setup(config, "activation_att_prob")
