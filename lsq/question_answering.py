...
from base_model import TransformerBase
...
class TransformerQuestionAnswering(TransformerBase):
    ...
    def train(self,
        ...
        best_dev_f1, best_dev_em= 0.0, 0.0

        ################## initialize the wgt-alpha #################
        logger.info("***** wgt_alpha initization! *****")
        self.model.apply(kl_divergence_initilization)
        #############################################################

        ####################initialize act-alpha here ######################################
        logger.info("***** activation alpha initialization *****")
        self.model.eval()
        self.model.apply(reset_act_alpha_true)
        for d in [dev_data_set]:
            self.evaluate_act_alpha(d, dev_features)
        self.model.apply(reset_act_alpha_false)
        ####################################################################################

        self.model.zero_grad()
#         train_iterator = trange(num_train_epochs, desc="Epoch")
        train_iterator = range(num_train_epochs)
        for _ in train_iterator:
#             epoch_iterator = tqdm(train_data_set, desc="Train iteration")
            for step, batch in enumerate(train_data_set):
                self.model.train()
                    ...
                    if logging_steps > 0 and global_step % logging_steps == 0:
                        ...
                        for d in ds:
                            ...
                            if results['f1'] > best_dev_f1:
                                ...
                                best_dev_em = results['exact']
                            logger.info('best_dev_f1 = %s', str(best_dev_f1))
                            logger.info('best_dev_em = %s', str(best_dev_em))
    ...
    def evaluate_act_alpha(self, dataset: DataLoader, features: InputFeatures):
        logger.info("***** Running inference for activation alpha initialization *****")
...
if __name__ == "__main__":
    ...
    data_dir = 'squad_data_model/data/squadv2.0' if args.version_2_with_negative else 'squad_data_model/data/squadv1.1'
    data_dir = os.path.join(args.root_dir,data_dir)
    model_dir = 'squad_data_model/models/tinybert_6l/squadv2.0' if args.version_2_with_negative else 'squad_data_model/models/tinybert_6l/squadv1.1'
    model_dir = os.path.join(args.root_dir, model_dir)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    n_gpus = torch.cuda.device_count()

    model = TransformerQuestionAnswering(model_type='quant_bert',max_answer_length=30,n_best_size=20,
                                        version_2_with_negative=args.version_2_with_negative,
                                        null_score_diff_threshold=0.0,max_query_length=64,
                                        model_name_or_path=model_dir,config_name=model_dir,tokenizer_name=model_dir
                                        ,do_lower_case=True,output_path=args.model_save_dir,device=device,n_gpus=n_gpus)

    tokenizer = model.tokenizer
    # train_file = 'train-v2.0.json' if args.version_2_with_negative else 'train-v1.1.json'
    # train_file = os.path.join(data_dir,train_file)
    # train_examples = read_squad_examples(train_file,True,args.version_2_with_negative)
    # train_data = model.convert_to_tensors(train_examples,False,max_seq_length=384,doc_stride=128)
    train_file = 'train-v2.0' if args.version_2_with_negative else 'train-v1.1'
    train_file = os.path.join(data_dir,train_file)
    with open(train_file, "rb") as reader:
    ...
    ############we modify it here################## check it with zhangwei, what role is it?
    # if args.version_2_with_negative:
    #     train_cls_index = torch.tensor([f.cls_index for f in train_features], dtype=torch.long)
    #     train_p_mask = torch.tensor([f.p_mask for f in train_features], dtype=torch.float)
    #     train_data = TensorDataset(train_input_ids, train_input_mask, train_segment_ids,
    #                             train_start_positions, train_end_positions,
    #                             train_cls_index, train_p_mask)
    # else:
    #     train_data = TensorDataset(train_input_ids, train_input_mask, train_segment_ids,
    #                             train_start_positions, train_end_positions)
    train_data = TensorDataset(train_input_ids, train_input_mask, train_segment_ids,
    ...
    dev_file = 'dev-v2.0.json' if args.version_2_with_negative else 'dev-v1.1.json'
    dev_file = os.path.join(data_dir,dev_file)
    dev_examples = read_squad_examples(dev_file,False,args.version_2_with_negative)
    dev_data, dev_features = model.convert_to_tensors(dev_examples,True,max_seq_length=384,doc_stride=128)
    dev_sampler = SequentialSampler(dev_data)
    dev_dataloader = DataLoader(dev_data, sampler=dev_sampler, batch_size=8)

    model.train(train_data_set=train_dataloader,dev_data_set=dev_dataloader,dev_examples=dev_examples,
                dev_features=dev_features,per_gpu_train_batch_size=16,data_dir=data_dir,logging_steps=200,
                num_train_epochs=4 if args.version_2_with_negative else 2)
